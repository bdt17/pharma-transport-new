...
Stripe.api_key = ENV['STRIPE_SECRET_KEY'] || 'sk_test_51...'
REDIS = Redis.new(url: ENV['REDIS_URL'] || 'redis://localhost:6379')

class PharmaTransportApp
  PRICES = { 'insulin' => 4900, 'vaccines' => 7900, 'biologics' => 12900 }.freeze
  ...
  def self.call(env)
    ...
    when ['POST', '/webhook'] then handle_webhook(req)
    else [404, {}, ['Not Found']]
    end
  end
  ...
  def self.process_payment(req)
    ...
    # Mock Stripe for now (add real keys later)
    stripe_url = "https://buy.stripe.com/test_#{type}_#{session_id}"
    
    REDIS.hset("session:#{session_id}", email: email, type: type, paid: 'true')
    
    [200, {'Content-Type' => 'application/json'}, 
     [{url: stripe_url, session_id: session_id}.to_json]]
  end

  def self.generate_pdf(req)
    session_id = req.params['session']
    data = REDIS.hgetall("session:#{session_id}")
    return [400, {}, ['Session expired']] unless data['paid'] == 'true'
