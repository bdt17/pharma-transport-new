...
# Thomas IT Pharma Transport - PHASE 23 PRO PORTAL (FINAL RENDER LIVE)
# FIXED: Proper Rack::Builder structure - 100% Render compatible
...
# ✅ PROPER RACK BUILDER - No top-level 'use'
app = Rack::Builder.new do
  # Custom session middleware FIRST
  class SimpleSessionMiddleware
    ...
    def call(env)
      ...
      session = load_session(request.cookies[SESSION_KEY])
      env['rack.session'] = session

      status, headers, body = @app.call(env)

      # Commit session if changed
      if session_changed?(env)
        new_session = env['rack.session']
        cookie = encode_session(new_session)
        headers['Set-Cookie'] = "#{SESSION_KEY}=#{cookie}; Path=/; HttpOnly; Secure; SameSite=Strict"
      end
    ...
    def load_session(cookie_value)
      ...
      begin
        data = Base64.urlsafe_decode64(cookie_value)
        json, signature = data.split('--', 2)
        return {} unless json && signature
        
        computed = OpenSSL::HMAC.hexdigest('SHA256', SECRET, json)
        return JSON.parse(json, symbolize_names: true) if secure_compare(signature, computed)
      rescue
      end
      {}
    end
    ...
    def session_changed?(env)
      env['rack.session'] && (env['rack.session'] != env['ORIGINAL_SESSION'])
    end
  end

  use SimpleSessionMiddleware
  ...
  class PharmaTransportProPortal
    ...
    def call(env)
      ...
      env['ORIGINAL_SESSION'] = @session.dup

      case @request.path
      when '/' then pro_landing_page
      when '/login' then handle_login
      when '/dashboard', '/history', '/generate_pdf' then require_auth { send(@request.path.sub('/', '_') + '_page') }
      when '/handle_payment' then handle_payment
      when '/fda_chain_of_custody' then [200, {'Content-Type' => 'text/html'}, [fda_chain_of_custody_html]]
    ...
    def _history_page
      [200, {'Content-Type' => 'text/html'}, [history_html]]
    end

    def _generate_pdf_page
      [200, {'Content-Type' => 'text/html'}, [pdf_html]]
    end
...
# Render.com compatible server
server = Rack::Handler::WEBrick
server.run app, Port: ENV['PORT'] || 3000, AccessLog: [], Logger: WEBrick::Log.new('/dev/null')
