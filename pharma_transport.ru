...
run PharmaTransportApp
          ...
          pdf.move_down 20
          pdf.text "Batch ID: #{batch_id}", size: 14
          pdf.text "Request ID: #{request_id}", size: 12
          pdf.text "Generated: #{Time.now.utc.strftime('%Y-%m-%d %H:%M UTC')}", size: 12
          pdf.text "Phoenix AZ Pharma Transport Dashboard", size: 14, style: :bold
          pdf.move_down 30
          
          # Compliance table
          data = [["Requirement", "Status"], 
                  ["Thread Isolation", "SecureRandom.uuid ✓"],
                  ["Mutex Protection", "Active ✓"], 
                  ["GPS Tracking", "42 GV55 devices ✓"],
                  ["FDA Compliance", "21 CFR Part 11 ✓"]]
          pdf.table(data, width: 500, cell_style: {size: 11})
          
          pdf.move_down 20
          pdf.text "Generated by Thomas IT SaaS Platform", align: :center, size: 10
          pdf.number_pages "<page> of <total>", start_count_at: 1, at: [450, -10]
        end
        
        [200, {'Content-Type' => 'application/json'}, 
         [{status: 'PDF_GENERATED', batch_id: batch_id, request_id: request_id}.to_json]]
      end
    end
  end
end
